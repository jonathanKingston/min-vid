...
function handleMessage(msg) {
  ...
  if (title === 'send-to-tab') {
    ...
    else if (opts.url) tabs.open(opts.url);
    else {
    ...
    store.queue.splice(opts.index, 1);
    ...
    store.queue.unshift(store.queue.splice(opts.index, 1)[0]);
    ...
    store.queue.concat(store.history.slice(0, 10));
