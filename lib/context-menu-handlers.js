...
const playNowLabel = _('context_menu_label_play');
const addToQueueLabel = _('context_menu_label_add');
...
self.on('click', function (node, data) {
  self.postMessage(node.href || node['data-feed-url'], data);
});`;

const contentScriptVimeoCase = `self.on('click', function (node, data) {
  self.postMessage('https://vimeo.com/' + node.href, data);
});`
...
function getMenu(opts) {
  menus.push(cm.Menu(Object.assign(opts, {
    label: 'Min Vid',
    image: self.data.url('img/gradient-logo.svg'),
    items: [
      cm.Item({label: playNowLabel, data: 'play', image: self.data.url('img/play.svg')}),
      cm.Item({label: addToQueueLabel, data: 'add-to-queue', image: self.data.url('img/add.svg'),})
    ]
  })));
}

function init() {
  const onMessageVimeoHandler = (url, data) => {
    launch({
      url: url,
      domain: 'vimeo.com',
  ...
  const onMessageYouTubeHandler = (url, data) => {
    launch({
      url: url,
      domain: 'youtube.com',
  ...
  getMenu({
    ...
    contentScript: `self.on('click', function (node, data) {
      self.postMessage(node.poster, data);
    });`,
    onMessage: function(url, action) {
